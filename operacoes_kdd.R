...
# Carregamento  em memória dos modelos já existentes
...
# 10 - Fatiar 15% da base total para reduzir o número de registros para treino (senão leva mto tempo pro algoritmo)
set.seed(7)
...
# 13 - Usar o modelo treinado, anteriormente, agora com a base de teste para testar a classificação
modelo_predito_rna <- predict(modelo_treino_rna, teste)
modelo_predito_id3 <- predict(modelo_treino_id3, teste)
# modelo_predito_glm <- predict(modelo_treino_glm, teste)

# 14 - Matriz de confusão para visualizar resultado da classificação com 
confusionMatrix(modelo_predito_rna, as.factor(teste$cidade))
confusionMatrix(modelo_predito_id3, as.factor(teste$cidade))
# confusionMatrix(modelo_predito_glm, teste$cidade_new)
...
# a) Remover outliers pela técnica de distância de Cook
